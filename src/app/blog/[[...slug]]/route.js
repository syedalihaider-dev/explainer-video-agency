const WORDPRESS_URL = "https://blog.explainervideoscompany.com/";
...
function buildPublicUrl(request, wordpressLocation) {
  try {
    ...
    if (
      locationUrl.hostname === "blog.explainervideoscompany.com" ||
      locationUrl.hostname === "www.explainervideoscompany.com" ||
...
async function proxyRequest(request) {
  ...
  forwardHeaders.set("Host", "blog.explainervideoscompany.com");
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (location) {
        try {
          ...
          if (locationUrl.hostname === "blog.explainervideoscompany.com") {
  ...
  if (
    ...
    // Match http:// or https:// blog.explainervideoscompany.com, EXCEPT for static/API paths
    const wpUrlRegex = /https?:\/\/blog\.explainervideoscompany\.com(?!(\/wp-(?:content|includes|json|admin|login)|xmlrpc))/g;
    const requestUrl = new URL(request.url);
